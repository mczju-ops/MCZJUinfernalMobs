...
它只在怪物进入冲锋阶段后，以临时长矛对本轮锁定玩家造成真实近战攻击时广播；距离接近本身不会触发。
`getDamage/setDamage` 读写的是这一次原始近战伤害，不会另外调用 `damage(...)` 产生第二次伤害。
取消 Hit 事件会取消对应的原始近战伤害；取消或将伤害改为 0 仍会消费本轮命中并立即结束持矛状态，
不会回滚 spear 的触发或冷却。
